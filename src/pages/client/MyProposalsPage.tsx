...
import { ServiceOrder, Proposal, ProposalStatus, ServiceOrderStatus } from '../../types';
import { InboxIcon, ChatBubbleLeftRightIcon, CheckCircleIcon, EyeIcon } from '../../components/icons/Icons';
import api from '../../utils/api';
import { toast } from 'sonner';

/*
 * MyProposalsPage – GET /api/serviceorder?clientId= + GET /api/proposal
 *
 * Data flow:
 *   On mount → api.get("/serviceorder", { params: { clientId: currentUser.id } })
 *     → Fetches all service orders placed by this client
 *
 *   On mount → api.get("/proposal")
 *     → Fetches all proposals, filtered client-side to show only those
 *       matching the client's service order IDs
 *
 * Proposals are grouped by job. Client can view and accept pending proposals.
 * Accepting a proposal updates its status via PATCH /api/proposal/:id.
 */
const MyProposalsPage: React.FC = () => {
  ...
  const [clientJobs, setClientJobs] = useState<ServiceOrder[]>([]);
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      try {
        ...
        setClientJobs(orders);

        // → Fetch all proposals, then filter for client's jobs
  ...
  // Group proposals by job ID
  const proposalsByJob = useMemo(() => {
    return clientJobs.reduce(
      (acc, job) => {
        const jobProposals = allProposals.filter((p) => p.serviceId === job.id);
  ...
  // → PATCH /api/proposal/:id to accept
  const confirmAcceptProposal = async () => {
    if (!selectedProposal) return;

    try {
      await api.patch(`/proposal/${selectedProposal.id}`, {
        status: ProposalStatus.ACCEPTED,
      });

      toast.success(`Proposal from ${selectedProposal.provider.name} accepted!`);
    ...
    <div className="space-y-8">
      ...
      {isLoading ? (
        ...
        <Card className="p-8 text-center">
          ...
          <p className="text-gray-500 mb-4">You haven't received any proposals for your active jobs.</p>
        ...
        jobEntries.map(({ job, proposals }) => (
          <Card key={job.id} className="p-6 shadow-lg mb-8">
            <h2 className="text-2xl font-poppins font-semibold text-primary mb-1">
              {job.service?.title || 'Untitled Job'}
            </h2>
            <p className="text-sm text-gray-500 mb-4">
              Posted: {new Date(job.createdAt).toLocaleDateString()} — Status:{' '}
              <span className={`font-medium ${job.status === ServiceOrderStatus.PENDING ? 'text-green-600' : 'text-yellow-600'}`}>
                {job.status}
              </span>
            </p>
                ...
                {proposals.map((proposal) => (
                  <Card key={proposal.id} className="p-4 border border-gray-200 hover:shadow-md transition-shadow">
                    <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center">
                      <div>
                        <div className="flex items-center mb-2 sm:mb-0">
                          ...
                          <div>
                            <h3 className="text-lg font-semibold text-gray-700">{proposal.provider?.name || 'Unknown'}</h3>
                      ...
                      <div className="text-right mt-3 sm:mt-0">
                        <p className="text-xl font-semibold text-primary">{proposal.bidAmount.toLocaleString()} FCFA</p>
                      </div>
                    </div>
                    ...
                    <div className="flex flex-wrap gap-2 mt-4 pt-3 border-t border-gray-200">
                      ...
                      <Button variant="ghost" size="sm" leftIcon={<ChatBubbleLeftRightIcon className="w-4 h-4" />}>
                        Chat
                      </Button>
      ...
      {selectedProposal && (
        ...
        >
          <p className="text-gray-700 mb-4">
            ...
            <span className="font-semibold">{selectedProposal.provider?.name}</span>?
          </p>
          <p className="text-lg font-semibold text-primary mb-4">
            Proposed Price: {selectedProposal.bidAmount.toLocaleString()} FCFA
          </p>
          <p className="text-sm text-gray-600 mb-6">
            Accepting this proposal will mark the job as "In Progress" and a contract will be created.
          </p>
          <div className="flex justify-end space-x-3">
            <Button variant="outline" onClick={() => setIsModalOpen(false)}>
              Cancel
            </Button>
            <Button variant="primary" onClick={confirmAcceptProposal}>
              Confirm & Accept
            </Button>
          </div>
        </Modal>
